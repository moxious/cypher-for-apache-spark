...
object StackOverflowExample extends App {
  ...
  // File-based CSV GDS
  caps.registerSource(Namespace("csv"), FileCsvGraphDataSource(rootPath = getClass.getResource("/csv").getFile))

  def query(): String =
    """
      |FROM GRAPH csv.stackoverflow
      |MATCH (a:Tag) RETURN a.name
    """.stripMargin
